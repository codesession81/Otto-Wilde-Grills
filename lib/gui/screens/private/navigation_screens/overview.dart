          ...
          children: <Widget>[
            ...
            Row(
              ...
              children: <Widget>[
                ...
                TextButton(
                  onPressed: (){
                    Navigator.push(context, MaterialPageRoute(builder: (context)=> ShowAllFavorities(memberList: Favorities.getFavoritiesList())));
                  },
                  child: Row(
                    children: const <Widget> [
            ...
            SizedBox(
              height: 220,
              child: ListView.builder(
                itemCount: Favorities.getFavoritiesList().length,
                scrollDirection: Axis.horizontal,
                itemBuilder: (context,index){
                  return Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Padding(
                        ...
                        child: ClipRRect(
                          ...
                          child: Stack(
                            children: <Widget>[
                              Image.network(Favorities.getFavoritiesList()[index].imgUrl,height: 150),
                              const Positioned(
                                right: 0,
                      ...
                      Row(
                        children: <Widget>[
                          ...
                          Text(Favorities.getFavoritiesList()[index].time,style:const TextStyle(color: Colors.grey),),
                        ],
                      ),
                      const SizedBox(height: 5),
                      Favorities.getFavoritiesList()[index].title.length > 14 ? Text(Favorities.getFavoritiesList()[index].title.substring(0, 14)+'...',style: const TextStyle(color: Colors.grey),):Text(Favorities.getFavoritiesList()[index].title,style:const TextStyle(color: Colors.grey)),
                      Row(
                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        children: <Widget>[
                          RatingBar.builder(
                            itemSize: 15,
                            initialRating: double.parse(Favorities.getFavoritiesList()[index].rating),
                            itemBuilder: (context,_)=> const Icon(Icons.star,color: Colors.yellow,),
                            onRatingUpdate: (rating){},
                          ),
                          Text("(${Favorities.getFavoritiesList()[index].people})",style:const TextStyle(color: Colors.grey))
                        ],
                      )
                    ],
                  );
